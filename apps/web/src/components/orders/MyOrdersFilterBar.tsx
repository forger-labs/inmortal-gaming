...
export default function MyOrdersFilterBar({
  ...
  return (
    <div className="flex flex-col gap-4 rounded-xl border border-white/10 bg-bg-surface/80 p-5 backdrop-blur-sm shadow-[0_4px_24px_rgba(0,0,0,0.3)]">
      {/* ─── Fila Superior: Busqueda, Estado y Orden ─── */}
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-12">
        {/* Busqueda por numero de orden o metodo */}
        <div className="flex flex-col gap-1.5 lg:col-span-4">
          <label
            htmlFor="my-orders-search"
            className="font-mono text-[11px] font-semibold uppercase tracking-wider text-text-secondary"
          >
            Buscar en mis pedidos
          </label>
        ...
        <div className="flex flex-col gap-1.5 lg:col-span-3">
          ...
          <select
            ...
            className="w-full rounded-md border border-white/10 bg-bg-primary px-3 py-2.5 font-body text-sm text-text-primary transition-all focus:border-neon-primary focus:outline-none focus:ring-1 focus:ring-neon-primary cursor-pointer"
          >
        ...
        <div className="flex flex-col gap-1.5 lg:col-span-2">
          <label
            htmlFor="my-orders-sort-filter"
            className="font-mono text-[11px] font-semibold uppercase tracking-wider text-text-secondary"
          >
            Orden cronologico
          </label>
          <select
            id="my-orders-sort-filter"
        ...
        <div className="flex flex-col gap-1.5 lg:col-span-3">
          <label
            htmlFor="my-orders-date-filter"
            className="font-mono text-[11px] font-semibold uppercase tracking-wider text-text-secondary"
          >
            Fecha de compra
          </label>
          <input
            id="my-orders-date-filter"
            type="date"
      ...
      {/* ─── Fila Inferior: Rango de montos y Reset ─── */}
      <div className="flex flex-col gap-4 pt-2 border-t border-white/5 sm:flex-row sm:items-end sm:justify-between">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex flex-col gap-1.5">
            <label
              htmlFor="my-orders-max-amount-filter"
              className="font-mono text-[10px] font-semibold uppercase tracking-wider text-text-muted"
            >
              Monto Max ($)
            </label>
            <input
              id="my-orders-max-amount-filter"
              type="number"
              min="0"
              step="any"
              placeholder="9999.00"
